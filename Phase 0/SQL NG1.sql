...
select distinct(school) as univ_maxsalary , max(salary) from teachers
group by univ_maxsalary

-- Case 2 Who is the teacher with the highest salary from Standford University ?
select * from teachers 
where school = 'Standford University' and salary = (select max(salary) from teachers) 

-- C.2 Join 
-- Case 1 Display all courses with teacher's identity
select teachers.first_name , teachers.last_name , courses.name , courses.teachers_id from teachers 
right join courses on teachers.id=courses.teachers_id 
order by courses.name asc

-- Case 2 Display how many courses per universities
select teachers.school, count(courses.name) from teachers 
...
select teachers.first_name, teachers.last_name , sum(courses.total_students) from teachers 
left join courses on teachers.id=courses.teachers_id 
group by teachers.first_name 

-- Case 4 Display how many courses per teachers
select teachers.first_name, teachers.last_name , count(courses.name) from teachers 
left join courses on teachers.id=courses.teachers_id 
group by teachers.first_name 

-- Case PPT Display teachers with the highest salary from each courses. Display all the teacher's and courses's attributes. Sort the results by the course's name ascending.
select * from teachers
right join courses on teachers.id = courses.teachers_id
